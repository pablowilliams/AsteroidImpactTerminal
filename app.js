...
const STRATEGIES = [
  {
    ...
    apply: (ctx) => {
      ...
      if (s20 == null || s50 == null) return { signal: "CLEAR", detail: "Not enough history for 50-day MA." };
      if (s20 > s50 * 1.005) return { signal: "WATCH",  detail: `20-day (${s20.toFixed(2)}) above 50-day (${s50.toFixed(2)}) — bullish trend.` };
      if (s20 < s50 * 0.995) return { signal: "MONITOR", detail: `20-day (${s20.toFixed(2)}) below 50-day (${s50.toFixed(2)}) — bearish trend.` };
      return { signal: "CLEAR", detail: `20-day ≈ 50-day — consolidation.` };
    },
  },
  {
    id: "rsi_mr",
  ...
  },
  {
    ...
    apply: (ctx) => {
      ...
      if (ratio > 1.3 && expectedReturn > 0.01) return { signal: "WATCH", detail: `Up/down ratio ${ratio.toFixed(2)}x, E[R] ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      if (ratio < 0.8 && expectedReturn < -0.01) return { signal: "MONITOR", detail: `Up/down ratio ${ratio.toFixed(2)}x, E[R] ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
  ...
  },
  {
    ...
    apply: (ctx) => {
      ...
      if (s.score > 0.25)  return { signal: "WATCH",  detail: `Net +${(s.score * 100).toFixed(0)} — crowd bullish.` };
      if (s.score < -0.15) return { signal: "MONITOR", detail: `Net ${(s.score * 100).toFixed(0)} — crowd bearish.` };
...
function combineSignals(results) {
  ...
  if (avg > 0.35)  return { signal: "WATCH",    detail: "Majority bullish." };
  if (avg < -0.35) return { signal: "MONITOR",   detail: "Majority bearish." };
...
function renderSentiment(stock) {
  ...
  const posts = s.posts.length ? s.posts : [{ sent: "neu", author: "@market_data", handle: "now", text: "No recent public chatter for this asteroid." }];
